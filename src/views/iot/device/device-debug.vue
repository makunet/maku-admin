  ...
  <div>
    <el-form v-loading="loading" :model="serviceDebugDataForm">
      <el-row :gutter="30">
        ...
        <el-col :span="8">
          <el-form-item label="指令" prop="command">
            <ma-dict-select v-model="serviceDebugDataForm.command" dict-type="device_command"
                            style="width: 100%"></ma-dict-select>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="30">
      ...
      <el-row :gutter="30">
        <el-col :span="8">
          <el-form-item>
            <el-button v-auth="'iot:device:send'" type="primary" @click="serviceDebugHandle(serviceDebugDataForm)">
              下发
            </el-button>
          </el-form-item>
        </el-col>
  ...
  <div>
    <el-form :model="propertyDataForm">
      <el-row :gutter="30">
        ...
        <el-col :span="8">
          <el-form-item label="属性" prop="propertyType">
            <ma-dict-select v-model="propertyDataForm.propertyType" dict-type="device_property"
                            style="width: 100%"></ma-dict-select>
          </el-form-item>
        </el-col>
      </el-row>
        ...
        <el-col :span="16">
          <el-form-item label="属性值" prop="payload">
            <template v-if="propertyDataForm.propertyType === 'RUNNING_STATUS'">
              <ma-dict-select v-model="propertyDataForm.payload" dict-type="device_running_status"
                              style="width: 100%"></ma-dict-select>
            </template>
      ...
      <el-row :gutter="30">
        <el-col :span="8">
          <el-form-item>
            <el-button v-auth="'iot:device:report'" type="primary"
                       @click="deviceReportAttributeDataHandle(propertyDataForm)">上报
            </el-button>
          </el-form-item>
        </el-col>
      </el-row>
...
import {
  useIot_deviceAsyncSendCommandApi,
  useIot_deviceSimulateDeviceReportAttributeDataApi,
  useIot_deviceSyncSendCommandDebugApi
} from "@/api/iot/device";
...
const serviceDebugHandle = (serviceDebugDataForm: any) => {
  if (serviceDebugDataForm.waitResponse == "1") {
    loading.value = true;
    useIot_deviceSyncSendCommandDebugApi(serviceDebugDataForm)
      .then(res => {
        ElMessage.success({
          message: res.data
    ...
    useIot_deviceAsyncSendCommandApi(serviceDebugDataForm).then(() => {
      ElMessage.success({
        message: "服务指令下发成功！"
      });
    });
...
const deviceReportAttributeDataHandle = (propertyDataForm: any) => {
  useIot_deviceSimulateDeviceReportAttributeDataApi(propertyDataForm).then(() => {
    ElMessage.success({
      message: "属性上报成功！"
    });
  });
