<template>
  <el-dialog v-model="visible" :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false">
    <el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="200px" @keyup.enter="submitHandle()">
      ...
      <el-form-item label="设备类型" prop="type">
        <ma-dict-select v-model="dataForm.type" dict-type="device_type" placeholder="设备类型"
...
import { useIot_deviceApi, useIot_deviceSubmitApi } from "@/api/iot/device";
...
const submitHandle = () => {
  dataFormRef.value.validate((valid: boolean) => {
    ...
    useIot_deviceSubmitApi(dataForm).then(() => {
      ElMessage.success({
        message: "操作成功",
        duration: 500,
        onClose: () => {
